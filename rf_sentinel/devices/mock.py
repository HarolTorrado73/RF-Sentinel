"""Dispositivo SDR mock para testing y desarrollo."""
...
class MockSDR(SDRDevice):
    """Dispositivo SDR simulado para testing."""
    ...
    @property
    def name(self) -> str:
        return f"MockSDR (device {self._device_index})"
    ...
    def read_samples(self, num_samples: int) -> bytes:
        ...
        iq = np.random.randn(num_samples * 2).astype(np.float32).tobytes()
        return iq
